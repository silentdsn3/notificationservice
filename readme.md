# **Задание: «Сервис непредсказуемых уведомлений»**
Создайте сервис, который отправляет пользователям уведомления не по расписанию, а на основе закономерностей их активности.
Например:
если пользователь пишет комментарии ночью три дня подряд — утром он получает сообщение «Кажется, вы сова 🦉»;
если пользователь покупает кофе по понедельникам, средам и пятницам — бот предлагает скидку во вторник.
Вы можете придумать любую собственную логику “умных” триггеров — чем оригинальнее и осмысленнее, тем лучше.

Технические требования
Язык: Java 17+
Фреймворк: Spring Boot
База данных: PostgreSQL (можно In-Memory, если схема продумана)
Использовать Gradle
REST API с методами:
POST /actions — добавить действие пользователя { userId, actionType, timestamp }
GET /notifications — получить все уведомления, сгенерированные системой

Важно: 
- не используйте cron-задачи (без шедулера/планировщика)
Система должна сама решать, когда “пора” реагировать на поведение пользователя.
Не используйте сторонние библиотеки, нам важно самостоятельное решение
Уделите внимание алгоритму, архитектуре и логике
